...
\chapter{Theory, specialized to $E=B=1$ SGD's test loss} \label{sect:calculus}
    ...
    \section{Both flat and sharp minima overfit less}
            ...
            \ref{fig:cubicandspring}\protect\offive{12}).
            %
            However, as SGD by
            ...
            \begin{figure}
                \centering
                \plotmoow{diagrams/sharp}{0.5\columnwidth}{}
                \caption{%
                   \textbf{
...
\chapter{Experiments}
    ...
    {\bf Experiments on natural and artificial losses.} The label
    \texttt{rvalue} refers to Theorem \ref{thm:resum}'s predictions,
    approximated as in Remark \ref{rmk:integrate}.  Curves marked
    \texttt{uvalue} are polynomial approximations to Theorem \ref{thm:resum}'s
    result (see \S\ref{appendix:sum-embeddings}).  \texttt{uvalue}s are simpler
    to work with but (see \protect\ofsix{4}) may be less accurate.
    ...
    \section{Training time, epochs, and batch size; $C$ repels SGD more than GD}
        ...
        \begin{figure} 
            \centering
            \pmoo{3.75cm}{neurips-test-small} 
            \pmoo{3.75cm}{neurips-test-large} 
            \caption{
        ...
        \begin{figure} 
            ...
            \pmoo{3.75cm}{neurips-gen-cifar-lenet} 
            \caption{
                \textbf{$C$ controls generalization\ gap and distinguishes GD
                from SGD.}
                %
...
\chapter*{Organization of the appendices}
    ...
    \par\indent     C.1 ~~ Long-term SGD prediction is \emph{PSPACE}-hard   \hfill \pageref{appendix:pspace}
    %\par\indent     C.3 ~~ Takeuchi's criterion, demystified                \hfill \pageref{appendix:tic}
    \par\indent     C.2 ~~ A new proof of the Chernoff bound                \hfill \pageref{appendix:chernoff}
...
\chapter{Bonus tracks}                   \label{appendix:bonus}
    ...
    \section{Long-term SGD prediction is \emph{PSPACE}-hard} \label{appendix:pspace}
        ...
        \begin{prop} \label{prop:pspace}
            ...
            landscape.
        \end{prop}
        This not-very-surprising proposition situates the short-term results of
        ...
        cycles.  Though the proposition itself is not surprising, perhaps this
        lemma is.
        ...
        \begin{proof}[Sketch of Proposition \ref{prop:pspace}'s proof]
            ...
            the subspace spanned by the $3+3+q+1$ axes spanned by the three
            participating tape-state axes, three participating head-location
            axes, all the head-state axes, and the clock axis.  The term is
            masked by a $3+3+q+1$-dimensional bump function so that when and
            only when we are nearby a point representing the input tuple do we
            move, completing one cycle around the clock, until we are close to
            the point representing the next tuple according to $\Mm$'s
            definition.  We use the Rachet lemma to shape this term so that
            the simulation of $\Mm$ goes forward in time.
