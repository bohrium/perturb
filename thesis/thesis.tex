...
\documentclass[notitlepage, justified]{tufte-book}
...
\begin{document}
    ...
    \chapter*{A Perturbative Analysis of Stochastic Gradient Descent}
    \section*{Samuel C.~Tenka}
    \section*{2020 July}
    ...
    \subsection*{Acknowledgements}
        ...
        We would like to thank \textsc{Sho Yaida}, \textsc{Dan A.\ Roberts},
        ...
        who introduced us to much of \S\ref{sect:related}.  Only with
        \textsc{Sho Yaida}'s advice to re-sum did our theory attain its most
        precise and conceptual form.
        %
        We appreciate the time and energy that \textsc{Andy Banburski},
        \textsc{Ben R.\ Bray}, \textsc{Jeff Lagarias}, and \textsc{Wenli Zhao}
        spent to critique our drafts.  In particular, \textsc{Wenli Zhao}
        nudged us to consider and discuss connections to physics, and
        \textsc{Ben R.\ Bray} taught us that gradient noise is rarely istropic,
...
\chapter{Introduction}
    ...
    (SGD) approximates noiseless gradient descent (GD) \citep{bo91}.  This
    thesis refines that intuition by showing how gradient noise biases learning
    toward certain areas of weight space.

    Leaving rigorous development to Appendix \S\ref{appendix:math}, we
    highlight in the thesis body our theory's intuition and main corollaries.
    ...
    $\sdia{c(01-2-3)(02-12-23)}$, analogous to those of \citep{fe49} and
    \citep{pe71}, depict these interactions. 
    %
    %-----------------  prospective  ------------------------------------------
    %
...
\section{Background, notation, and assumptions} \label{sect:background}
...
\section{Related work} \label{sect:related}
    ...
    It was \citep{ki52} who, in uniting gradient descent \citep{ca47} with
    ...
    \citep{ne17a}.  For example, \citep{ba17} controls the Rademacher complexity
    ...
    by SDE to analyze implicit regularization (e.g.\ \citep{ch18}), but, per
    \citep{ya19a}, such continuous-time analyses cannot treat SGD noise
    correctly.
    ...
    \citep{ro18}; unlike that work, we generalize beyond order $\eta^1$ and
    $T=2$.
    ...
    generalize (\citep{ho17}, \citep{ke17}, \citep{wa18}), or by showing that
    \emph{sharp} minima generalize (\citep{st56}, \citep{di17}, \citep{wu18}).
    ...
    Prior work analyzes SGD perturbatively: \citep{dy19} perturb in inverse
    ...
    \citep{ch18} and \citep{li17} are forced to assume uncorrelated Gaussian
...
\chapter*{Organization of the appendices}
    ...
    \par\indent     C.7 ~~ Additional marginfigures                               \hfill \pageref{appendix:marginfigures}
...
\chapter{Experimental methods}\label{appendix:experiments}
    ...
    \subsection{Additional marginfigures}                                 \label{appendix:marginfigures}
