...
\chapter{Introduction}
    ...
    \section{Background on Learning} \label{sect:background}
        ...
        Learning involves mapping a training sequence $\Ss \sim \Dd^N$ to a
        hypothesis $p\in \Cc$ that is accurate at test time.
        More precisely, we posit a set $|\Dd|$ of
        ...
        $\Ein = \EE_{\Ss} \circ E$.  Observe that if $p\in \Cc$ is a
        hypothesis-valued random variable independent from $\Ss$, then
        $\Ein(p)$ is an unbiased estimator of $\Eout(p)$; the conclusion
        fails when $p, \Ss$ are dependent, a phenomenon we know as
        \textbf{overfitting}.
        ...
        \Yy^\Xx$.  In practice, they arise as compositions of simple
        ...
        \Hh\to \Yy$ or $\tilde{A}:\Hh\to\Xx\to\Yy$, where each $A(x)$ is
        smooth.\footnote{
            Concretely, $\tilde{A}(p)$ is typical of the form
            $$
                a_{100} \circ f_{99}(p) \circ a_{98} \circ
                               \cdots \circ f_1(p) \circ a_0 
            $$
            where $a, f$ are smooth and each $f$ is bilinear.
        }  We also posit an objective funcion written in
        curried form as $O:|\Dd|\to\Yy\to\RR$, with each $O(d)$ smooth.
        Then the error function, restricted to $\Hh$, is 
        $E(p)(d) = O(d)\circ \tilde{A}(p)$ for $p\in \Hh$ and $d\in \Dd$.
        This error function $E:\Hh\to |\Dd|\to\RR$ alternatively curries to
        a \textbf{loss landscape} $l:|\Dd|\to\Hh\to\RR$, which we regard as
        parameterizing a random variable $l_d: \Hh\to\RR$ valued in the smooth
        functions.\footnote{
            Caution: we will overload notation by using $l$ to mean $l_d$'s 
            expectation.
        }
        ...
        gradient descent.  Explicitly, we choose an initial point $p_0\in \Hh$
        and then repeatedly update using learning rate $\eta$:
        $$
            p_{t+1} = p_t - \eta (\nabla \Ein)(p) 
        $$
        Observe that $p$ takes values in $\Hh$ while $(\nabla \Ein)(p)$ takes
        ...
        } Then $-\eta (\nabla \Ein)(p)$ is a vector, and we may flow along this
        vector geodesically from $p_t$ to $p_{t+1}$. 

        The special case of linear image classification illustrates the
        metric's role in learning.  Here, $\Xx$ is a 
        vector space\footnote{
            All of our spaces will be finite-dimensional unless
            obviously otherwise.
        } of images and $\Hh = \Xx^\star$ is the space of linear features.
        Since $\Hh$ and $\Xx$ are dual, we may regard whatever inverse metric
        on $\Hh$ we use for descent as a metric on $\Xx$.  If, intuitively,
        each feature is a question that can be asked of an image, then the
        metric $\eta:\Xx\to\Xx^\star$ turns an image $x$ itself into a
        question, namely the question of ``is this image like $x$?''  Crucially,
        different choices of $\eta$ will yield different questions for the same
        $x$ ---
        %
        ranging from
        \emph{is this image like $x$ in that both are blue near the center?}
        (for a center-masked pixel-wise metric)
        to
        \emph{is this image like $x$ in that their edges are aligned?}
        ...
        \subsection{Stochastic Gradient Descent}
            ...
            $\Ein = \sum_{d\in\Ss} l_d/|\Ss|$ or more generally $l_\Bb =
            \sum_{d\in\Bb} l_d/|\Bb|$ for non-empty \textbf{batches}
            ...
            may perform more small-batch updates per unit time.

            Though it may be efficiency in optimization that motivates smaller,
            randomly chosen batches, we will show that such \emph{stochastic
            gradient descent} (SGD) biases learning away from certain regions
            of weight space, and we discuss when and why this helps
            generalization.
            %
            Overall, we find that SGD differs from descent on $\Ein$, not only
            through diffusion terms sublinear ($\sqrt{T}$) in the number $T$ of
            updates but also through a linear drift that scales with $T$.
