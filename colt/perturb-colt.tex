...
\begin{document}
    ...
    \section{Introduction}
        ...
        %\subsection{SGD}
            ...
            the primary learning signal when training deep neural nets.
            While users of deep learning benefit from the intuition that
            such \emph{stochastic gradient descent} (SGD) approximates
            deterministic gradient descent (GD) \citep{bo91,le15}, SGD's
            gradient noise in practice alters training dynamics and hence
            testing losses \citep{go18, zh19, wu20}.  This paper characterizes
            these dynamics and shows that gradient noise biases learning toward
            low-curvature, low-noise regions of weight space.

            %--  vs ode and sde  ----------------------------------------------
            %
            Departing from \cite{li18,we19b,zh19}, we model
            correlated, non-gaussian, anisotropic gradient noise and
            ...
            Taylor-expand the testing loss as a function of the learning rate
            $\eta$, taming large-time convergence issues by re-summing to a sum
            over topologically-characterized processes.  E.g.\ 
            an instance of the process\footnote{
            ...
            processes, a technique that may inspire future theoretical
            advances.
            %
            %This diagram technique is potentially powerful.
            %\S\ref{appendix:tutorial} offers a tutorial in our diagram
            %technique to promote future inquiry.
            %Overall, this process-based perspective may inspire future
            %theoretical inquiries beyond the \S\ref{sect:consequences}'s
            %results.
    ...
    \section{Perturbative theory of SGD}\label{sect:calculus}
        ...
        \subsection{Diagrams arise from Taylor series and depict information-flow processes}\label{sect:diagrams}
            ...
            \begin{dfn}[\S\ref{appendix:toward-diagrams}]
                \emph{A \textbf{diagram} is a rooted tree equipped with a partition
            ...
            \begin{table}[h!]
                ...
                \begin{tabular}{ccc}
                    examples: of valid diagrams\footnote{
                Throughout, colors help us refer to parts of diagrams; colors
                lack mathematical meaning.
            } &\hspace{0.2cm}& of invalid diagrams \\ \hline
                    $\mdia{c(0-1)(01)},
                    \mdia{c(0-1-2)(02-12)},
        ...
        \subsection{Diagrams overcome the challenges of using Taylor series to study SGD}\label{sect:using}
            ...
            this divergence.%, thus establishing diagrams as a necessary
            %concept.
            ...
            \end{wrapfigure}
            \noindent
            Intuitively, our notion of ``the same'' permits chains to grow or
            shrink (see Definition \ref{dfn:link}).  We will express in closed
            form the total contribution to (\ref{eq:dyson}) of all diagrams in
            such a list.  The idea is that the uvalues of chains are powers of
            hessians --- for example,
            $\uvalue(\sdia{MOOc(0-1-2-3-4)(01-12-23-34)}) = GH^3G$ --- so we
            may sum over all possible chain lengths via geometric series.
            %
            This is the \emph{resummed value} (\emph{rvalue}) of the list's
            unique linkless member.

            \begin{dfn}\label{dfn:link}
                \emph{A \textbf{link} is a degree-$2$ non-root node that participates in
                no gray ties.  To \emph{reduce} at a link, we
            ...
            \noindent
            We define the \textbf{rvalue} of an embedded diagram 
            as the $\uvalue$ except that pairs of tensor indices are
            ...
            (For example, take Figure \ref{fig:resumintuition}'s embedding
            of $\sdia{c(0-1)(01)}$ (topmost of four).  The associated uvalue  
            is $G_\mu\eta^{\mu\nu}G_\nu$: a $G$ for each degree-one node and
            an $\eta$ for each edge.  By contrast, the associated rvalue is
            $G_\mu((I-\eta H)^{12-1}\eta)^{\mu\nu}G_\nu$ since the edge spans
            $12$ timesteps.  We see that this rvalue includes uvalues for embeddings
            of $\sdia{c(0-1-2)(01-12)}$, etc.)
    ...
    \section{Consequences of the theory}\label{sect:consequences}
        ...
        %\end{samepage}
            \begin{wrapfigure}{r}{3.3cm}
                ...
                \vspace{-0.20cm}
                \caption{
                    Chladni drift on $\Mm=\RR^2$.  Red arrows show
        ...
        \subsection{SGD descends on a landscape smoothed by the current $C$}
            ...
            derivative but $C\nabla H$ is not.  By low-pass filter theory, 
            $l$ increases by $CH/2!+o(C)$ when we convolve $l$ with a
            $C$-shaped Gaussian, so we say that SGD descends on a $C$-smoothed
    ...
    \section{Conclusion}
        ...
        \subsection{Related work}\label{sect:related}
            ...
            Other studies focus on \emph{double descent} and suggest that some
...
\section{Tutorial: how to use diagrams}                \label{appendix:tutorial}
    ...
    of losses.  
        ...
        \subsection{Two example calculations}
            ...
            \subsubsection{Leading order effect of gradients}
                ...
                \begin{quest}\label{qst:grad}
                    What is the leading order contribution of the testing loss's gradient
                    ($G$) to the final testing loss $\expc l(\Theta_T)$?  
    ...
    %MOOO
    \begin{landscape}
        \subsubsection{Computations: Grids}
            We {\bf specify, as a grid}, the batch size, training set size, and
            number of epochs of the setting under analysis. 
            That is, we take an $N\times T$ grid and shade its cells, shading
            the $(n,t)$th cell when the $t$th update involves the $n$th data
            point.  Thus, each column contains $B$ (batch size) many shaded
    ...
    \begin{landscape}
        ...
        cells to which they are assigned; we draw the root nodes outside the
    ...
    \begin{landscape}
        \subsubsection{Computations: Evaluating each diagram embedding}
            In these examples, we choose to compute $\uvalue$s instead of
            $\rvalue$s.  The former are an approximation
            appropriate when $T$ is fixed.
            \par \indent $\bullet$
            ...
            \par \indent $\bullet$
                ...
                the covariance: $\expc[xy]-\expc[x]\expc[y]$.\footnote{
                    %If the part contains three nodes $x,y,z$, then the cumulant
                    %bracket is
                    %$
                    ...
                    $$
                        \expc[\prod x_i] - \sum_{\text{partition}~P} \prod_{p\in P} \CC[\prod_{i\in p} x_i]
                    $$
                }
            \par \indent $\bullet$
        ...
        \begin{tabular}{p{0.48\linewidth}p{0.48\linewidth}}
          ...
          &
            ...
            and single-epoch spacetimes: its multi-epoch
    ...
    \begin{landscape}
        ...
        Our Key Lemma('s restatement) says that to compute a testing loss, we
        multiply each diagram's $\uvalue$ by the number of ways that
        diagram embeds, where embeddings with $s$ many symmetries count only
        $1/s$ much toward the total number of embeddings.  A symmetry of an embedding
        $f$ of a diagram $D$, i.e.\ an element of $\Aut_f(D)$, is defined to be
        a relabeling of $D$'s nodes that simultaneously preserves $D$'s rooted
        tree structure, $D$'s partition structure, and $f$'s assignment of nodes
        to $(n,t)$ cells of the grid.  This is a strong constraint, so there
        will typically be no symmetries except for the identity, meaning that
        $s=1$. 
        %    \begin{equation*}
        %        \sum_{\substack{D~\text{a} \\ \text{diagram}}}
        %        ~
