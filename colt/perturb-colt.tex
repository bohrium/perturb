...
\begin{document}
    ...
    \section{Perturbative theory of SGD}
        ...
        \subsection{Introducing diagrams}\label{sect:diagrams}
            ...
            but a few diagrams will be irrelevant.  E.g.\ if $E=B=1$, then
            a diagram with an ancestor-descendant pair in the same part
            contributes zero.  For fixed $T$, to order $\eta^d$ we may
            neglect all diagrams with more than $d$ edges.
            %The $t^d T^{-p}$-th order correction\footnote{We compare ODE
            %integrated to time $t$ to $T$ steps of SGD with $\eta =
            %\eta_\star t/T$ and $E=B=1$, and we assume $p\neq 0$.} to
            %the ordinary differential equation approximation of SGD is
            %given by diagrams with $d$ edges and $p$ many gray ties.
            For SGD initialized at a minimum of $l$, all diagrams vanish
            that contain a leaf participating in no gray tie.)

        \subsection{Using diagrams}

            The topology of diagrams organizes our evaluation of
            (\ref{eq:dyson}). 

            %-----  embeddings and patterns of influence  ---------------------

            {\color{red} TODO: define (and draw?) embeddings!}
            ...
            %-----  resummation  ----------------------------------------------
                ...
                Each of the following lists contains diagrams of
                ``the same'' shape:
                \vspace{-0.33cm}
                \begin{align*}
                    \sdia{c(0-1)(01)},
                    ...
                    \mdia{MOOc(03-1-2-4)(01-12-24-34)},\cdots
                \end{align*}
                \vspace{-0.67cm}
