...
\begin{document}
    ...
    \section{Perturbative theory of SGD}
        ...
        \subsection{Discussion of a trivial example}
        ...
        \subsection{Introducing diagrams}\label{sect:diagrams}
            ...
            %
            %\begin{rmk}
                (One may draw dozens of small diagrams.  In many analyses, all
                but a few diagrams will be irrelevant.  For instance, if
                $E=B=1$, then a diagram with an ancestor-descendant pair in the
                same part contributes zero.  When performing a fixed-$T$
                calculation to order $\eta^d$, we may neglect all diagrams with
                more than $d$ edges.  The $t^d T^{-p}$-th order
                correction\footnote{%
                    We compare ODE integrated to time $t$ to $T$ steps of SGD
                    with $\eta = \eta_\star t/T$ and $E=B=1$, and we assume
                    $p\neq 0$.
                }
                to the ordinary differential equation approximation of SGD is
                given by diagrams with $d$ edges and $p$ many gray ties.  For
                SGD initialized at a minimum of $l$, all diagrams vanish that
                contain a leaf participating in no gray tie.)
            %\end{rmk}

        \subsection{Insights from diagrams}

            While \S\ref{appendix:future} discusses bridges to physics and
            relations to Hessian methods and natural GD, this section discusses
            how the topology of diagrams organizes our evaluation of
            (\ref{eq:dyson}). 

            %-----  embeddings and patterns of influence  ---------------------

            %-----  isolate effect of tensors  --------------------------------

            %-----  crossing symmetries  --------------------------------------

            %-----  resummation  ----------------------------------------------

                Intuitively, the four diagrams in each of these lists share
                an essential shape:
                \vspace{-0.33cm}
                \begin{align*}
                    \sdia{c(0-1)(01)},
                    ...
                    \mdia{MOOc(0-1-2-3-4)(01-12-23-34)}
                    &&
                    \sdia{c(01-2)(02-12)},
                    \sdia{c(02-1-3)(01-13-23)},
                    \mdia{MOOc(02-1-3-4)(01-14-23-34)},
