...
\section{Tutorial: how to use diagrams}                \label{appendix:tutorial}
    ...
    \subsection{Two example calculations}
        We illustrate the four-step diagram paradigm by using diagrams to
        answer the following two questions.
        \subsubsection{Question A: leading order effect of gradients}
        ...
        \subsubsection{Question B: leading order effect of epochs}
            Our second example calculation reproduces (an illustrative case of)
    ...
    \begin{landscape}
        ...
        \begin{tabular}{p{0.15\linewidth}p{0.40\linewidth}p{0.40\linewidth}}
            \textsc{Step}&\textsc{Effect of Gradients (Question \ref{qst:grad})}&\textsc{Effect of Epochs (Question \ref{qst:multi})}\\
            \hline
          % Specify Grid
            A.\ {\bf Specify, as a grid}, the batch size, training set size,
            and number of epochs. 
            %A cell at row $n$ and column $t$ is shaded provided that the $n$th
            %training sample inhabits the $t$th batch.
            We take an $N\times T$ grid and shade its cells, shading the
            $(n,t)$th cell when the $t$th update involves the $n$th data
            point.  Thus, each column contains $B$ (batch size) many shaded
            cells and each row contains $E$ (epoch number) many shaded
            cells.
          &
            Question \ref{qst:grad} does not specify a batch size, epoch
            number, or training set size and so does not specify a grid.  In
            fact, we wish to answer the Question for any choice of those
            hyperparameters.  For instance, we'll answer the Question for
            $B,E,N=2,4,8$ (grid shown below) and also for
            $B,E,N=1,2,8$ (grid shown below).
            \begin{center}
            \par\noindent\parbox{0.90\linewidth}{
                \begin{center}
                \dmoo{2.75cm}{spacetime-b2-e4-nosh}
                \end{center}
                \par
                    ...
                    timesteps. 
                \begin{center}
                \dmoo{2.75cm}{spacetime-b1-e2-shuf}
                \end{center}
                \par
                    \textbf{A grid for SGD} with $B=1$, $E=2$, $N=8$, $T=16$.
                    This example, unlike the previous, depicts
                    inter-epoch shuffling: that is,
                    the $b$th batch in 
                    one epoch differs from the $b$th batch in a different epoch.
            }
            \end{center}
          &
            ...
            \begin{center}
            \par\noindent\parbox{0.90\linewidth}{
                \dmoo{2.75cm}{spacetime-b1-e2-nosh}
                \hfill
                \dmoo{2.75cm}{spacetime-b1-e1-nosh}
                \par
    ...
    \end{landscape}
    \begin{landscape}
        \subsubsection{Computations: Embeddings of diagrams into a grid}
        \begin{tabular}{p{0.15\linewidth}p{0.40\linewidth}p{0.40\linewidth}}
            \textsc{Step}&\textsc{Effect of Gradients (Question \ref{qst:grad})}&\textsc{Effect of Epochs (Question \ref{qst:multi})}\\
            \hline
            ...
            %\begin{itemize}
                \par \indent \textbf{time-ordering condition}: the times $t$ strictly increase 
                    along each path from leaf to root; and
                \par \indent \textbf{correlation condition}: if two nodes are in the same
            ...
            cells to which they are assigned.  
            &
            &
