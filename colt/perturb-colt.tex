...
\begin{document}
    ...
    \section{Introduction}
        ...
        \subsection{Intuitions about SGD}
            ...
            } loss landscape for which SGD's trajectory perpetually
            \emph{circulates} counterclockwise; slightly modifying this
            ...
            %
            We thus compute quantities such as expected testing losses by
            summing small sets of diagrams.  Overall, our analysis offers and
            rigorously grounds a new physics-inspired perspective --- of SGD as
            a superposition of many concurrent information-flow processes ---
    ...
    \section{Perturbative theory of SGD}
        ...
        \subsection{An exegesis of a trivial example}
            We illustrate our approach by justifying Proposition
            \ref{prop:nest}.
            %
            Intuitively, each step of SGD displaces $\theta$ by $-\eta G$ and
            ...
            \end{proof}
            This paper extends the above proof by keeping higher order terms.
            We emphasize three features that the above shares with its na\"ive
            generalizations: the ``\emph{transparency}'' of future terms to old
            effects, the combinatorial \emph{explosion} of intermediate terms,
            and the results' \emph{polynomial} dependence on $T$.
            ...
            analpgue of independence that finds precise expression in Theorem
            \ref{thm:resum}.  The intuition that arises is that of many
            independent processes occuring, each blind to all but $d$
            updates.

            When we expand to higher order, an \textbf{explosion} of terms
            rapidly obstructs progress.  In a second order analysis, for
            instance, we may not discard the hessian term
            \colorbox{moosky}{$\eta\nabla\nabla l_{T^\pr}(\theta_0) \cdot
            (\theta_{T^\pr} - \theta_0)$}.  The first-order contributions of
            all updates prior to $T^\pr$ thus each contribute via $\theta_{T^\pr}$
            to this undiscarded term.  So, while the proof above sums
            $O(T)$ terms, a second order analysis sums $O(T^2)$
            terms, each (e.g.\ $\nabla\nabla l_{t=5} \nabla l_{t=2}$) involving
            a \emph{pair} of times.  In fact, the terms may vary in form:
            some (perhaps $\nabla\nabla l_{t=5} \nabla l_{t=2}$) of the
            quadratic terms that replace \colorbox{moopink}{$\expc[\theta_T -
            \theta_0]$}
            have statistically independent factors that permit expectations to
            factor, while others (perhaps $\nabla\nabla l_{t=5} \nabla
            l_{t=5}$) will not.  This is how covariances of noise appear in
            our analysis.
            ...
            and vary
            $T$:\footnote{%
                There is no contradiction here: the domain and rate of
                convergence (of the Taylor series in $\eta$) typically depend
                on $T$, so we do not expect $\lim_{\eta\to 0}$ and
                $\lim_{T\to\infty}$ to commute.%
            }
            for example, SGD on a bounded loss function should not experience
            unbounded loss as $T$ increases.  By contrast, our main results
            ...
            resummation without omission or redundancy of terms so
            that absolute convergence ensures consistency.  The physical
            interpretation is to replace ``\emph{transparency}'' by
            ``\emph{translucency}'': we find that the strength of a term
            involving updates separated in time decays exponentially in the
            separation.

        \subsection{Perturbation as technique}
            We now abstractly describe the terms that appear when we keep
            higher order terms in our proof of Proposition \ref{prop:nest}.
            %
        ...
        \subsection{Introducing diagrams}\label{sect:diagrams}
            Though we here define diagrams, the following examples of valid
            and invalid diagrams by themselves provide a sufficient sense to
            ...
            \end{table}
            \par\noindent
            (One may draw dozens of small diagrams.  In many analyses,
            all but a few diagrams will be irrelevant.  For
    ...
    \section{Consequences of the theory}
        ...
        \subsection{Non-Gaussian noise affects SGD but not SDE}
            ...
            Even if we restrict to single-epoch SGD, differences arise due to
            time discretization and non-Gaussian noise; the following corollary
        ...
        \subsection{Both flat and sharp minima overfit less}
            ...
            \begin{figure}[h!]
                ...
                \plotmooh{colt/cubic}{}{0.40\columnwidth}
                \plotmooh{colt/spring}{}{0.40\columnwidth}
                \caption{%
    ...
    \section{Experiments}
        ...
        \subsection{Training time, epochs, and batch size; $C$ repels SGD more than GD}
            ...
            \begin{figure}[h!] 
                ...
                \caption{
                    ...
                    \S\ref{appendix:sum-embeddings}).  \texttt{uvalue}s are
                    simpler to work with but (see\protect\ofsix{4}) may be
                    less accurate.
                    %%%
                    %%%
                    \newline {\bf Left: Perturbation models SGD for small $\eta
                    ...
                    %%%
                    %%%
                    \newline {\bf Center: $C$ controls generalization gap and
                    distinguishes GD from SGD.}
                    %
                    ...
                    %%%
                    %%%
                    \newline {\bf Right: Predictions near minima excel for
                    large $\eta T$.} \protect\ofsix{4}: SGD travels \Helix'
                    ...
                    very flat $H$.  Near $H=0$, our predictions improve on TIC
        ...
        \subsection{Minima that are flat \emph{with respect to} $C$ attract SGD}
            ...
            that, from $C(\theta)$'s perspective but not globally, is more
            attractive (flatter).  This permits eternal motion (into the page)
            ...
            \begin{figure}[h!]
                ...
                \caption{%
                    ...
                    the valley of minima intersects each pane's center.  Dotted
                    blue curves help to compare adjacent panes.  Red arrows
                    show the major axis of gradient noise in each pane.  
                    %
    ...
    \section{Conclusion}
        ...
        optimization on short timescales or near minima.  Corollaries
        \ref{cor:entropic} and \ref{cor:overfit} together offer insight into
        SGD's success in training deep networks: SGD avoids curvature and
        noise, and curvature and noise control generalization.
        ...
        %Thus, the theory might help to analyze fine-tuning (e.g.\
        %\cite{fi17}'s MAML).
    ...
    \section*{Organization of the appendices}
        ...
        %Many questions remain.  \S\ref{appendix:future} briefly discusses
        %bridges to physics and their relations to Hessian methods and natural
        %GD.
