...
\section{Tutorial: how to use diagrams}                \label{appendix:tutorial}
    ...
    \begin{landscape}
        ...
        \begin{tabular}{p{0.48\linewidth}p{0.48\linewidth}}
            ...
            choice of those hyperparameters.  For instance, we'll answer the
    ...
    \begin{landscape}
        \subsubsection{Computations: Evaluating each diagram embedding}
            ...
            appropriate when $T$ is fixed.% instead of taken to
            %infinity.  %$\uvalue$s have the same asymptotic error as $\rvalue$s
            %with respect to $\eta$.
            %Moreover, $\uvalue$s are simpler to calculate,
            %since they depend only on diagrams, not on embeddings.  So to
            %compute a testing loss, we multiply each diagram's $\uvalue$ by the
            %number of ways that diagram embeds.
            \par \indent $\bullet$
