...
\begin{document}
    ...
    \section{Paradigmatic example: a non-conservative entropic force}\label{sect:entropic-example}
        ...
        %\subsection{SGD}
            ...
            for each natural $k\geq 0$ is a
            compact set $K_n$ so that for each $\theta\in \Mm$
            and $l_x\in \Dd$: $\nabla^n l_x(\theta)\in K_k$. 
            Here, $\nabla^n$ denotes the $k$th derivative operator; $\nabla^k
            l_x(\theta)$ is thus a tensor with $k$ axes.
            %
            \textbf{Analytic Moments}:
            any polynomial $p$ of the $0$th and higher derivatives of $l_x$
            induces a random (function-valued) variable whose expectation
            $\expct{p}:\Mm\to\RR$ (exists and) is analytic; moreover,
            the infimum over $\theta\in \Mm$ of $\expct{p}$'s radius
            of convergence at $\theta$ is strictly positive.
            %
            These assuptions imply that $\nabla \expct{p}=\expct{\nabla p}$.

            We quote a well-known proposition
        ...
        \subsection{Taylor series: method and challenges}
            ...
            \begin{proof} %(of Proposition \ref{prop:nest}).
                ...
                \begin{align*}
                    ...
                    &= \textstyle\text{\colorbox{moolime}{$- \sum_\nu \eta^{\mu\nu} \nabla_\nu l_{{\tilde T}}(\theta_0)$}} + o(\eta^1)
                \end{align*}
                ...
                \begin{align*}
                    ...
                    &= \textstyle \sum_\mu \nabla_\mu l(\theta_0) \text{\colorbox{moopink}{$\expc[\theta_T - \theta_0)]^\mu$}} + \expc[o(\theta_T - \theta_0)] \\
                    &= \textstyle \sum_\mu \nabla_\mu l(\theta_0) (-T\eta G + o(\eta^1)) + o(O(\eta^1)) \\
                    &= \textstyle \text{\colorbox{moogold}{$- \sum_{\mu\nu} T G_\mu \eta^{\mu\nu} G_\nu$}}+ o(\eta^1)
                \end{align*}
                Indeed, due to analytic moments, the above expectations of
                $o(\eta^1)$ terms are still $o(\eta^1)$.
            \end{proof}

            Now, what happens when we keep higher order terms?  We used above
            that, to first order in $\eta$, $\expct{l(\theta_T)}$ depends on the
            training data only through the first moment
            \colorbox{moopink}{$\expc[\theta_T - \theta_0]$}.\squish\squish\ But to compute  
            ...
            Complications arise even as we compute $M_1$\squish\ to order $\eta^2$.  We may not neglect the gradient
            correction $\nabla \wrap{\text{\colorbox{moosky}{$\nabla
            ...
            summands, each (e.g.\ $\nabla\nabla l_{5} \nabla l_{2}$)
            involving a \emph{pair} of times.
            %
            Therefore, to order $\eta^1$, the displacement $\sum_{\tilde T}
            \wrap{\text{\colorbox{moolime}{$- \eta \nabla l_{{\tilde
            T}}(\theta_0)$}}}$\squish\squish\ is degree $1$ in $T$; the order-$d$ displacement is
            degree $d$.

            To obtain \colorbox{moogold}{$-TG\eta G$},\squish\ we multiplied
            $l$'s derivatives by the expectations of such summands.
            %
            In contrast to the Prop, these expectations, even those of a fixed
            ...
            l_x$'s higher cumulants appear in our analysis.
            
            At order $\eta^3$, a hessian correction $(\theta_{{\tilde T}} -
            \theta_0) \cdot \nabla \nabla l_{n_{\tilde T}}(\theta_0) \cdot
            (\theta_{{\tilde T}} - \theta_0)/2$ augments the gradient
            correction.
            %
            Then $M_1$'s order-$\eta^3$ summands vary in form,
            even when all expectations factor (i.e., noiseless landscapes).
            E.g.\ the hessian and gradient corrections respectively contain
            summands such as
            \squash
            $$
                \textstyle
        ...
        \subsection{Some interesting terms; Displacement}

            \textbf{Minima that are flat \emph{with respect to} $C$ attract SGD}
        ...
        \subsection{A non-conservative entropic force}
            %
            ...
            velocity field with non-zero curl.
            ...
            $\tilde{l}$.  The non-zero curl in SGD's velocity field shows that
            ...
            \newpage
    \section{Background, notation, assumptions}\label{sect:background}
            ...
            $1+o(1)$}.
            %with convergence not necessarily uniform
            %w.r.t.\ $T$.
            %A quantity $q$ \emph{vanishes to order $d$} when $\lim_{\eta\to 0}
            %q/p(\eta) = 0$ for some homogeneous degree-$d$ polynomial $p$; we
            %then say $q\in o(\eta^d)$.  THIS SENTENCE CONFUSES!
    ...
    \section{Perturbative theory of SGD}\label{sect:calculus}
        ...
        \subsection{Diagrams arise from Taylor series and depict information-flow processes}\label{sect:diagrams}
            ...
            %\colorbox{moolime}{\emph{\textbf{uvalue}}, defined as a product containing a $d$th}
            %\colorbox{moolime}{derivative of $l_x$ for each degree-$d$ node, grouped under
            %expectation symbols per the diagram's gray}
            %\colorbox{moolime}{outlines, and tensor-contracted per the diagram's black edges}
            ...
            \begin{dfn}[\S\ref{appendix:toward-diagrams}]
                \emph{A \colorbox{moolime}{\textbf{diagram}} is a rooted tree equipped with an
    ...
    \section{Conclusion}
        ...
        \subsection{Related work}\label{sect:related}
            ...
            \begin{figure}[H]
                ...
                \end{tabular}                                                                                                        
                %\begin{tabular}{c}                                                                                                  
                                        %   $\ww{\rRr{\AAA l_x} \rRr{\BBB l_x}} \ww{\gGg{\CCC \DDD      l_x}} \ww{\bBb{\EEE \FFF l_x}}$\\
                                        %   $\ww{\rRr{\AAA l_x} \rRr{\BBB l_x}} \ww{\gGg{\CCC \DDD \EEE l_x}} \ww{\bBb{     \FFF l_x}}$\\
                                        %   $\ww{\rRr{\AAA l_x} \rRr{\BBB l_x}      \rRr{\CCC \DDD \EEE l_x}} \ww{\gGg{     \FFF l_x}}$
                %\end{tabular}                                                                                                       
                \begin{tabular}{c}
                    $\rightsquigarrow$\\
                    $\rightsquigarrow$\\
                ...
                \end{tabular}
                \begin{tabular}{c}                                                                                                   
                                            $\ww{\rRr{\AAA l_x} \rRr{\BBB l_x}} \ww{\gGg{\AAA \DDD      l_x}} \ww{\bBb{\BBB \DDD l_x}}$\\ 
                                            $\ww{\rRr{\AAA l_x} \rRr{\BBB l_x}} \ww{\gGg{\AAA \BBB \EEE l_x}} \ww{\bBb{     \EEE l_x}}$\\
                                            $\ww{\rRr{\AAA l_x} \rRr{\BBB l_x}      \rRr{\AAA \BBB \EEE l_x}} \ww{\bBb{     \EEE l_x}}$
                \end{tabular}                                                                                                        
                %\begin{tabular}{c}                                                                                                  
                                        %   $\ww{\rRr{\AAA l_x} \rRr{\BBB l_x}} \ww{\gGg{\CCC \DDD      l_x}} \ww{\bBb{\EEE \FFF l_x}} \eta^{\mu\lambda} \eta^{\nu\rho}   \eta^{\sigma\pi}$\\
                                        %   $\ww{\rRr{\AAA l_x} \rRr{\BBB l_x}} \ww{\gGg{\CCC \DDD \EEE l_x}} \ww{\bBb{     \FFF l_x}} \eta^{\mu\lambda} \eta^{\nu\sigma} \eta^{\rho\pi}$\\
                                        %   $\ww{\rRr{\AAA l_x} \rRr{\BBB l_x}      \rRr{\CCC \DDD \EEE l_x}} \ww{\gGg{     \FFF l_x}} \eta^{\mu\lambda} \eta^{\nu\sigma} \eta^{\rho\pi}$
                %\end{tabular}
                \caption{
                    \textbf{Def.\ 
                    \ref{dfn:uvalue-body}}.
                    Left: diagrams.
                    Middle: incorporate node-degree information. 
                    Right: uvalues.
                }
