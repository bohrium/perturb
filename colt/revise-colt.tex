    ...
    \section{Perturbative theory of SGD}\label{sect:calculus}
        ...
        \subsection{Main result}\label{sect:main}
            ...
            \begin{thm} \label{thm:converge}
                ...
                constant-$M$ or constant-$N$ SGD initialized close to
            ...
            \begin{rmk}
                \emph{
                %Even without Local Strength, 
                We have not in practice observed the pathologies (of
                non-commuting limits, i.e., of consequentially non-uniform
                little-$o$s) that might plague Thm \ref{thm:converge}'s
                expression.  We take as a physically plausible and empirically
                supported hypothesis that Thm \ref{thm:resum} predicts SGD's
                behavior in the settings of \S\ref{sect:consequences} for
                practical (finite but large) $T$s and landscapes.}\mend
...
\section{Experimental methods}\label{appendix:experiments}
    ...
    \section{Review of Tensors}
        ...
        the role of types in Haskell or C.  One could do entirely without
        types, but to do so would hinder interpretation of the
        raw bitstrings we manipulate and would obscure which operations
        (e.g.\ dereferencing of a pointer; concatenation of strings) are
        natural and which are not (e.g.\ dereferencing of a float;
        concatenation of file pointers). 
        %
        The language of tensors likewise supplies our analysis with
        abstractions and algebraic operations appropriate to derivatives and
        moments in high dimensional loss landscapes.

        For example, we immediately recognize $C^{-1} + H$ as ill-formed, even
        though in our formalism both $C^{-1}$ and $H$ are square grids of
        ...
        \subsection{Vectors versus covectors}
            ...
            $\RR$.  When $V$ is clear from context, we call elements of $V$
            \emph{vectors} and elements of $V^\star$ \emph{covectors}.\footnote{
                ...
                \emph{vector}\footnote{a.k.a.\ \emph{column} a.k.a.\
                \emph{displacement} a.k.a.\ \emph{primal} vector} is a linear
                map of type $\RR\to V$ and a \emph{covector}\footnote{a.k.a.\
        ...
        \subsection{What is a tensor?}
            ...
            We capture the essential aspects of the above by defining what it
            means for a vector space $P$ to be a tensor product of given vector
            spaces $V, W$; a `tensor' is then just an element of $P$.
            %
