...
\begin{document}
    ...
    \section{Paradigmatic example: a non-conservative entropic force}\label{sect:entropic-example}
        ...
        \subsection{Notation and assumptions}\label{sect:setup}
            %--  the landscape  -----------------------------------------------
            ...
            %Thus, $\nabla \expct{p}=\expct{\nabla p}$.
            %
            We \textbf{sometimes} (each time explicitly) also assume
            ...
            depends on neither $x$ nor $\theta$), this estimate is exact.\squash\squash
        ...
        \subsection{Taylor series: method and challenges}
            ...
            summands, each (e.g.\ $\nabla\nabla l_{5} \nabla l_{2}$)
            involving a \emph{pair} of times.  In general, order-$d$
            ...
            T}}(\theta_0)$}}}$\squish\squish\ is degree $1$ in $T$; but the order-$d$ displacement is
            degree $d$ in $T$.
            ...
            \subsubsection{Diagrams in brief}
        ...
        \subsection{An entropic force with curl}\label{subsect:entropic}
            ...
            $J_{\omicron\pi\rho} = \nabla_\omicron H_{\pi\rho}$.%is $l$'s `jerk' (\href{https://www.iso.org/obp/ui/\#iso:std:iso:2041:ed-3:v1:en}{ISO 2041 (2009)}, \S1).
            %\footnote{
            ...
            \begin{wrapfigure}{r}{0.3\textwidth}
                ...
                \caption{%
                    ...
                    %\textbf{Left}:
                    {Gradient noise pushes SGD toward minima flat w.r.t.\ $\mathbf{C}$.}
                        \small
                        A 2D loss near
                        ...
                        %\ref{cor:entropic}.
                        %We label but do
                        %not assume \cite{we19b}'s `fast' vs `slow' modes.
                        %C.f.\ Figure \ref{fig:archimedes}.
                        %
                    %\textbf{\bf Right}:
            ...
            %
            It is the bilinear form
            $F^{\mu\nu}=\textstyle\sum_{\xi\omicron}\eta^{\mu\xi}
            \eta^{\nu\omicron} C_{\xi\omicron}$ that determines which hessians
            count as `large' or `small'.  For example, if $F$
            ...
            %}
            %In sum: minima \emph{that are flat w.r.t.\
            %$C$} attract SGD.
            %
            ...
            dominates near $l$'s minima when
            $E=1$: with ${\mathcal P}_T(s) = \lim_{s^\pr\to s}(1 - \exp(-Ts^\pr)))/s^\pr$,\squash\squash
            %
            \begin{cor}%[Computed from $\sdia{c(01-2-3)(02-12-23)}$]
                \label{cor:entropic}
                Start SGD at a minimum of $l$ with $H>0$, $E=B=1$ and use
                an eigenbasis of $K=\eta H$.  
                For any $T$, the displacement is %$\expct{\theta_T-\theta_0}^\xi$ is 
                \squash
                $$
                    M_1^\xi = %\expct{\theta_T-\theta_0}^\xi = 
                    -
                    {\textstyle\sum_{\substack{\mu\nu\xi \\ \omicron\pi\rho}}}
                        ...
                        \eta^{\mu\nu}\eta^{\omicron\pi}
                        J_{\nu\pi\xi}
                        {\color{gray}{\mathcal P}_T(K_{\xi\xi})}\eta^{\rho\rho}/2
                    + o(\eta^3)
                    \squash\squash
                $$
                Assuming Local Strength (\S\ref{sect:setup}), the error
                decays uniformly, i.e.,
        ...
        %\subsection{Curl in the entropic force}
            \begin{figure}[h!]
                ...
                \plotmooh{colt/screw-trajectory-cropped}{}{0.17\textwidth} 
                \plotmooh{plots/neurips-thermo-linear-screw}{}{0.17\textwidth}
                \squash
                \caption{%
                    ...
                    large $\eta T$.  %SGD traverses \Helix'
                    %valley.
