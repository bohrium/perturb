...
\begin{abstract}
    ...
    %
    We harness Feynman Diagrams to reason about Stochastic
    Gradient Descent (SGD) at small learning rates $\eta$.
    %
    %-----------------  applications  -----------------------------------------
    %
    We derive dynamical laws allowing us to: 
        construct a regularizer causing large-batch GD to emulate
        small-batch SGD;
        %
        exhibit a non-conservative entropic force driving SGD;
        %
        generalize the Akaike Info.\ Criterion (AIC) to a smooth
        quantity liable to descent; and
        %
        quantify how SGD differs from the popular approximation
        SDE. 
    %
    %-----------------  mention of experiments  -------------------------------
    %
    We verify our predictions on artificial data and convnets for CIFAR-10 and
    Fashion-MNIST.
\end{abstract}
...
\section{Introduction}
    ...
    predict how SGD drifts from $\theta$ through macroscopic timescales, e.g.
    $\eta T$ large enough to decrease error by $0.5$ percentage points.
...
\section{Experiments}
    ...
    \subsection{Sharp and flat minima both overfit less} \label{subsect:overfit}
        ...
        implicit in gradient descent to threshold from sharp minima\footnote{
            ...
            prediction of generalization gap.
        }.
        %
...
\section{Conclusion} \label{sect:concl}
    ...
    We thus reconcile prior intuitions that sharp \citep{ ke17, wa18} or 
    ...
    SGD moves toward regions  with respect to $C$.  As $C$ evolves, the
    ...
    \subsection{Consequences}
        ...
        Our results may also help to analyze and design fine-tuning procedures
        such as the meta-learning of MAML (\cite{fi17}).  Indeed, those methods
        strive for models initialized to pre-trained weights and tunable to new
        data through a small number, $T$, of updates, a setting matched to the
        ...
        weight.  For example, we proved that SGD moves toward regions  with
        ...
        in closed-form dynamical laws valid valid in each short-term patch of
        SGD's trajectory.
    ...
    \subsection{Questions}
        ...
        \begin{conj}[Riemann Curvature Regularizes]
            ...
            grows.
        \end{conj}
        Though our work so far assumes a  metric $\eta^{\mu\nu}$, it
...
\section{Glossary}\label{sect:glossary}
    ...
    \begin{description}
        ...
        \item[entropic force] a macroscopic force arising from a tendency
            ...
            entropic force, SGD tends toward minima that are  with respect
            to the covariance. 
        ...
        \item[spring] the familiar simple machine with energy quadratic in its
            ...
            Takeuchi prediction that  minima generalize divergently badly.
    ...
    \begin{tabular}{p{0.1\textwidth}p{0.45\textwidth}p{0.4\textwidth}}
           \textsc{Diagram}             &  \textsc{Unre-summed Value}                                                & \textsc{Interpretation} \\ \hline
