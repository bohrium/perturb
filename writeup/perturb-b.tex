...
\section{Introduction}
    %\subsection{Overview}
        ...
        A key question is how such noise affects the final objective value.  We
        address this question by connecting SGD dynamics to physical scattering
        theory.
    ...
    \subsection{Appetizing Example: The Role of Epoch Number}
        ...
        We represent an SGD run as a ``\emph{spacetime}''
        grid with one row per datapoint and one
        column per time.  The grid has a cell at a row and column whenever the
        corresponding datapoint is part of the corresponding time's batch.
        Our formalism represents an
        SGD run's expected test loss as a \emph{sum over all diagram
        embeddings}, where diagrams are graphs such as $\sdia{(0-1)(01)}$ and
        $\sdia{(01-2)(02-12)}$ and $\sdia{(01-2)(01-12)}$ composed of thin
        edges and fuzzy ties.  Diagrams evaluate to numbers as later described.
        \emph{Embeddings} of diagrams into spacetime grids are maps of nodes
        into cells that preserve left-right relationships along thin edges and
        that map two nodes into the same row exactly when they are fuzzily
        tied.  Each $d$-edged diagram contributes to order $\eta^d$, so,
        for small $\eta$, the few-edged diagrams suffice to predict
        test losses.
        ...
        \ref{sect:compare} shows that to avoid them can convolute analyses.   
...
\section{Background and Notation}
    ...
    \subsection{Tensor Conventions}
        ...
        respect to optimization dynamics.  We use $G_\mu, H_{\mu\nu},
        J_{\mu\nu\lambda}$ for the first, second, and third derivatives of $l$.
        ...
        are weak enough to hold for $\tanh$ neural networks with cross entropy
        loss on bounded data --- and with arbitrary weight sharing, skip
        connections, soft attention, dropout, batch normalization\footnote{
            assuming batches remain disjoint throughout training so that each
            batch may be considered as a single datapoint in a
            higher-dimensional space 
        } and weight decay.  Moreover, our analysis does not rely on
        information-geometric relationships between $C$ and $H$, so it applies
        to inexact-likelihood landscapes such as VAEs'.

    %--------------------------------------------------------------------------
    %           Names of SGD Parameters
    %--------------------------------------------------------------------------

    \subsection{SGD Conventions}
        SGD decreases an objective $l$ by updating on smooth, unbiased i.i.d.
        estimates $(l_n: 0\leq n<N)$ of $l$.  For each of $M\times B$ epochs,
        SGD partitions the train set into $N/B$ batches of size $B$; for
        each batch $\Bb$, SGD rewrites
        $
            \theta^\mu
            \leftsquigarrow
            \theta^\mu -
            \eta^{\mu\nu} \nabla_\nu \frac{1}{B} \sum_{n\in \Bb} l_n(\theta)
        $.
        In total, SGD performs $NM$ many updates.  Together with an inter-epoch
        shuffling pattern, $N, B, M$ determine the SGD algorithm.  The cases
        $B=1$ and $B=N$ we call \emph{pure SGD} and \emph{pure GD}.  The $M=1$
        case of pure SGD we call \emph{vanilla SGD}.
...
\section{Conclusion}
    ...
    presented can by means of diagrams be done fully in one's head.
        ...
        before affecting a final test loss.  In fact, we import physical tools
        such as \emph{crossing symmetries} \citep{dy49b} and
...
\section{Mathematical Background} \label{sect:background}
