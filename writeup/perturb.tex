...
\section{Introduction}
    %\subsection{Overview}
        ...
        by a Feynman diagram\footnote{
            ...
            %Prior knowledge of physics is not prerequisite. 
...
\section{Diagram Calculus for SGD} \label{sect:calculus}
    ...
    %    {\color{moor} EXCISED FOR SPACE... BRIEFLY OVERVIEW?}
...
\section{Tutorial on Diagram Rules} \label{sect:tutorial}
    ...
    After reviewing the diagram method's recipe step by step, we will work through two
    examples in using diagrams to analyze descent.  As in the main text, we
    ...
    \subsection{Listing Relevant Diagrams}
        ...
        \subsubsection*{Review of Rules}
            ...
            Moreover, Theorem \ref{thm:sgdcoef} says that the test loss involves
            only diagrams whose root (a.k.a. rightmost node) participates in no
            fuzzy ties.
            And it says that the generalization gap (ie. test minus train loss)
            involves only diagrams whose root participates in a fuzzy
            outline with one other node.
            \begin{rmk*}
        ...
        \subsubsection*{Examples}
            ...
            \end{quest*}
            The $d$-edged diagrams contribute to order $eta^d$, so we wish to
            enumerate the diagrams with one edge.  Per Theorem
            \ref{thm:sgdcoef}, the generalization loss is a sum over diagrams
            whose root (a.k.a.  rightmost node) participates in one fuzzy
            outline.  So we seek $1$-edged diagrams whose rightmost node is
            fuzzily outlinedly tied to one other node.  It turns out there is
            just one such diagram: $\sdia{c(01)(01)}$.  Indeed, there is only
            one possible rooted tree of thin edges.  And since there are only
            two nodes, the rightmost node's fuzzy-outline partner is
            determined.
            \begin{rmk*}
            ...
            \end{quest*}
            The $d$-edged diagrams contribute to order $eta^d$, so we wish to
            enumerate the diagrams with two edges.  Per Theorem
            \ref{thm:sgdcoef}, the test loss is a sum over diagrams whose root
            (a.k.a. rightmost node) participates in no fuzzy ties.  So we seek
            $2$-edged diagrams whose rightmost node isn't fuzzily tied.  It
            turns out there are four such diagrams:
            $\sdia{(0-1-2)(01-12)}$,
            $\sdia{(01-2)(01-12)}$,
            $\sdia{(0-1-2)(02-12)}$, and
            $\sdia{(01-2)(02-12)}$.  Indeed, two permissible rooted trees
            multiply with two permissible fuzzy tie patterns to yield four
            diagrams.
            ...
            \begin{quest*}
                ...
                loss at order $\eta^2$? 
            \end{quest*}
            We seek $2$-edged diagrams whose rightmost node isn't fuzzily tied.  As
            we saw before, there are four such diagrams.  However, consideration of
            one-epoch, singleton-batch SGD's spacetime shows us that no node in the
            thin-edge tree may be fuzzily tied to any of its descendents.  Thus, of
            the four possible diagrams (namely, $\sdia{(01-2)(01-12)}$), one has no
            ...
            \end{quest*}
            We seek $2$-edged diagrams (order $\eta^2$) whose rightmost node
            has a fuzzy outline connecting to one other node (generalization
            gap).  We ignore trees whose fuzzy-outline-erased versions involve
            fuzzy ties between any pair of ancestor and descendant (one-epoch).
            To be explicit, we write out the fuzzy outlines as the differences
            they represent: 
            $\sdia{(0-12)(01-12)}-\sdia{(0-1-2)(01-12)}$,
            $\sdia{(0-12)(02-12)}-\sdia{(0-1-2)(02-12)}$, and
            $\sdia{(012)(02-12)} -\sdia{(01-2)(02-12)} $.

    \subsection{Interpreting a Diagram}
        {\color{moor} FILL IN}

    \subsection{Integrating a Diagram over Spacetime}
        \subsubsection*{Unrenormalized (simpler, but bad for large $\eta T$)} 
        \subsubsection*{Renormalized (good for large $\eta T$)} 
