...
\section{Introduction}
    ...
    \subsection{Appetizing Example: The Role of Epoch Number}
        ...
        compare the expected test losses of one-epoch and multi-epoch SGD (say,
        with learning rates scaled proportionally).  In our formalism, these
        test losses are the \emph{sum over all diagrams embedded in spacetime},
        ...
        Appendix \ref{sect:compare} shows that to avoid them can lead to
...
\section{Background and Notation}
    ...
    analysis, we regard the learning rate as an inverse metric $\eta^{\mu\nu}$
    that converts a gradient into a displacement (\cite{bo13}).  We use $\eta$
    ...
    expressions transform naturally with respect to optimization dynamics.  We
    use $G_\mu, H_{\mu\nu}, J_{\mu\nu\lambda}$ for the first, second, and third
    derivatives of $l$.

    We henceforth assume the regularity conditions listed in Appendix
    \ref{sect:proofs}, for instance that $l$ is analytic.  The conditions are
    weak enough to hold for $\tanh$ neural networks with cross entropy loss on
    bounded data --- and with arbitrary weight sharing, skip connections, soft
    attention, dropout, batch normalization\footnote{
    ...
    information-geometric relationships between $C$ and $H$, so it applies to
    ...
    \subsection{Forms of SGD}
        ...
        $.
        In total, SGD performs $NM$ many updates.
        %We consider SGD on $N$ points with batch size $B$ for
        %$M\times B$ epochs.
        Together with an inter-epoch shuffling pattern,
        $N, B, M$ determine the SGD algorithm.
        The cases $B=1$ and $B=N$ we call \emph{pure SGD} and \emph{pure GD}.
        The $M=1$ case of pure SGD we call \emph{vanilla SGD}.
        
        %The pattern of updates is
...
\section{Conclusion}
    ...
    The diagram method's physical metaphor invites further exploration of
    Lagrangian characterizations and curved backgrounds: 
    \begin{quest}
