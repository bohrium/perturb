...
\section{Introduction}
    ...
    \subsection{Example of diagram-based computation of SGD's test loss} \label{subsect:example}
        ...
        In fact, \S\ref{appendix:interpret-diagrams} physically interprets
        edges as carrying information between updates and toward
        ...
        \begin{exm} \label{exm:first}
            ...
            each edge (here, there are three edges):
        ...
        \begin{rmk}
            ...
            change in curvature $H$ w.r.t.\ $\theta$.  So skewed noise affects
            SGD in proportion to the logarithmic derivative of curvature.
        \end{rmk}
...
\subsection{Background, notation, and assumptions} \label{sect:background}
    ...
    %\subsection{Loss landscape}
        ...
        %
        So our theory models $\tanh$ networks with cross entropy loss on
    ...
    %\subsection{SGD terminology}
        ...
        $,
        where $\Bb_t$, the $t$th batch, is sampled without replacement.
        For simplicity, we will assume unless otherwise stated that
...
\section{Theory, specialized to $E=B=1$ SGD's test loss} \label{sect:calculus}
    ...
    %\subsection{Diagrams, embeddings, and re-summed values}
        ...
        corresponding sequence of times is increasing.  For instance, $f$ might
        send $\sdia{c(01-2-3)(03-12-23)}$'s red part to $t=3$ and its green
        part to $t=4$, but not vice versa.
        %
        ...
        brackets $\expc_x{}$.
        %
        \textbf{Edge rule}: for each edge whose endpoints are sent by $f$ to
        times $t, t^\prime$, insert a factor of $K^{\wabs{t^\prime-t}-1} \eta$
        where $K \triangleq (I-\eta H)$.
        %
        So if $f$ maps $\sdia{c(012-3)(03-13-23)}$'s red part to time $t =
    ...
    \subsection{Insights from the formalism}
        ...
        minima flat w.r.t.\ $C$.}
            ...
            the green (an extra $\nabla$ on $H$), to $D$'s degree-$1$ root,
            $G$.  By l'H\^opital,\footnote{
                %
                If a displacement $\Delta \theta$ increases loss by $C\nabla H
                G$ nats, and by $G$ nats per meter, then $\Delta \theta$ is
                $C\nabla H$ meters.
                %
            } the displacement is $\propto C\nabla H$.  That is, SGD moves
            toward minima that are flat w.r.t.\ $C$ (Figure
            \ref{fig:cubicandspring}\ofthreemoo{0}).
            %
            Taking limits to drop the non-degeneracy hypothesis, we expect
            sustained motion toward flat regions in a valley of minima.  By
            avoiding \cite{we19b}'s assumptions of constant $C$, we find that
            SGD's velocity field is typically non-conservative, i.e.\ has curl
            (\S\ref{subsect:entropic}).  Indeed, $\nabla(CH)$ is a total
            derivative but $C\nabla H$ is not.  Since $CH/2+o(C)$ is the loss
            increase upon convolving $l$ with a $C$-shaped Gaussian, we say
            that SGD descends on a $C$-smoothed landscape that changes as $C$
            does.
            %
        ...
        \subsubsection{Both flat and sharp minima overfit less}
            ...
            \begin{cor}[from $\sdia{c(01-2)(02-12)}$, $\sdia{c(01)(01)}$]\label{cor:overfit}
                ...
                $$
            \end{cor}
            Figure \ref{fig:cubicandspring}\protect\ofthreemoo{12} illustrates
            this result's intuition.  The gen.\ gap tends  
            ...
            \begin{figure}[h!]
                ...
                \caption{%
                    ...
                    \textbf{Left}:
                        ...
                        \emph{w.r.t.\ the covariance} (Corollary
                        ...
                        the valley.  $J = \nabla H$ measures how 
                        curvature changes across the valley.
                    \textbf{\bf Right}:
                        Both curvature and the structure of noise affect
                        overfitting.
                        %$\leftrightarrow$ shows weights; $\updownarrow$ shows
                        %loss.
                        \protect\ofthreemoo{1}:
                        \emph{covector}-perturbed landscapes favor large $H$s.
                        \protect\ofthreemoo{2}:
        ...
        \subsubsection{High-$C$ regions repel small-$(E,B)$ SGD more than large-$(E,B)$ SGD}
            \label{subsect:epochs-batch}

            Intuition (\S\ref{appendix:interpret-diagrams}) suggests that noise
            repels SGD.  This effect is strongest for small $E, B$:
            ...
            \begin{cor}[$\sdia{c(01-2)(01-12)}$] \label{cor:batch}
                SGD's weight avoids high $C$s more than GD's:
                $
                    l_{C}
                        \triangleq
                    \frac{N-1}{4 N}
        ...
        \subsubsection{Non-Gaussian noise affects SGD but not SDE}
            ...
            popular theoretical approximation to SGD, but SDE and SGD differ in
            ...
            time discretization and non-gaussian noise. 
            %
...
\section{Evaluation: correctness and significance of results}
...
\subsection{Experiments}
    ...
    \subsubsection{Training time, epochs, and batch size; $C$ repels SGD more than GD}
        ...
        \begin{figure}[h!] 
            ...
            \caption{
                ...
                Since SGD's motion depends smoothly on the landscape, the
                special case of \Archimedes\ establishes that
                non-conservativity is generic.    
                %
    ...
    \subsubsection{Minima that are flat \emph{with respect to} $C$ attract SGD}
        \label{subsect:entropic}
        To test that ``flatness'' depends on $C$, \S\ref{appendix:artificial}
        constructs a landscape, \Archimedes, with non-constant $C$ throughout
        its valley of global minima.  As in Archimedes' screw or
        Rock-Paper-Scissors, each point $\theta$ has a neighbor that,
        from $C(\theta)$'s perspective but not absolutely, is flatter.  This
        permits eternal cyclic motion.
        %
        Corollary \ref{cor:entropic} predicts 
        a $z$-velocity of $+\eta^2/6$ per timestep, while \cite{ch18}'s
        SDE-based analysis predicts a velocity of $0$.\footnote{
...
\subsection{Conclusion: implications for practice} \label{sect:concl}
    ...
    %\subsection{Consequences}
        ...
        (small $\eta T$) in general.  Thus, our theory might especially
        help to analyze meta-learners that are based on fine-tuning (e.g.\
        \cite{fi17}'s MAML).
...
\section*{Acknowledgements}
    ...
    %\begin{ack}
        ...
        their patient guidance.  We appreciate how
            \textsc{Andy Banburski},
            \textsc{Ben R. Bray},
            \textsc{Jeff Lagarias}, and
            \textsc{Wenli Zhao}
        took time to critique our drafts.
...
\section*{Organization of the appendices}
    ...
    {\bf
    \par\noindent A ~ How to calculate expected test losses}                \hfill {\bf page \pageref{appendix:tutorial}}
    ...
    \par\indent     B.1 ~~ Setup and assumptions                            \hfill \pageref{appendix:assumptions}
    ...
    \par\indent     B.7 ~~ Proofs of miscellaneous claims                   \hfill \pageref{appendix:claims}
