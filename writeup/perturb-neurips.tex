...
%   Redacted during double-blind peer review.
    ...
    %\begin{ack}
        ...
        was he who introduced us to much of \S\ref{sect:related}.  Only with
        \textsc{Sho Yaida}'s advice to re-sum did our theory attain its most
        precise and conceptual form.
        %
        We appreciate the time and energy that
            \textsc{Andy Banburski},
            \textsc{Ben R.\ Bray},
            \textsc{Jeff Lagarias}, and
            \textsc{Wenli Zhao}
        spent to critique our drafts.
        In particular, \textsc{Wenli Zhao} nudged us to consider and discuss
        connections to physics, and \textsc{Ben R.\ Bray} taught us
        that gradient noise is rarely istropic, homogeneous, or Gaussian.  
        %
...
\section*{Organization of the appendices}
    ...
    \begin{itemize}[noitemsep]
        ...
        \item to precisely prove our results (and pose a conjecture);
    ...
    \par\indent     B.1 ~~ Assumptions                                      \hfill \pageref{appendix:assumptions}
...
\section{How to calculate test losses: a practical guide}\label{appendix:tutorial}
    ...
    initialization.  Here, we explain this technique.  Note that new
    combinatorial objects --- \emph{space-time grids} --- arises as we relax
    the paper body's assumption that $E=B=1$.  This, too, we will explain.  We
    note for now that there are are {\bf four steps} to computing the expected
    test loss, or other quantities of interest, after a specific number of
    gradient updates: 
    \begin{itemize}
        ...
        \item {\bf Draw embeddings}, of diagrams into the
            space-time grid, as needed for our desired precision.
    ...
    detail each of the steps in its own sub-section.  We then discuss the
    intuition that diagrams offer.  Though we focus on the computation of
    expected test losses, we describe how the four steps also give us
    variances, train losses, of weight displacements.  Finally, we note that
    our theory may be phrased without reference to diagrams; we illustrate on
    sample problems when and why diagrams streamline computation.
    ...
    \subsection{An example calculation: the effect of epochs}       \label{appendix:example}
        ...
        Specifically, a routine and explicit check shows that, on a noiseless
        linear landscape $l_x=l \in (\RR^n)^*$, the versions
        attain equal test losses, namely $l(\theta_0) - T l_\mu \eta^{\mu\nu}$.
        %
        Thus, the answer to Question \ref{qst:multi} will vanish to first order
        in $\eta$; that is, the answer will be second-order (or higher-order)
        in $\eta$.

        \subsubsection*{Space-time grids}
            We encode the training set size, batch size, and number of epochs
            by considering a set of pairs $(n,t)$, with one pair for each
            participation of the $n$th datapoint in the $t$th update.  It will
            be convenient to draw these $(n,t)$ pairs as shaded cells in an
            $N\times T$ grid.  Thus, each column contains $B$ (batch size) many
            shaded cells, and each row contains $E$ (epoch number) many shaded
            cells.  This shaded grid is the SGD's \emph{space-time} or
            \emph{space-time grid}.
            To answer Question \ref{qst:multi}, we consider two space-time
            grids: one for multi-epoch SGD and another for single-epoch SGD.
            Figure \ref{fig:spacetimes-epoch} depicts these two grids.
            \begin{figure}[h!] 
                \centering
        ...
        \subsubsection*{Embeddings of diagrams into space-time}
            A \emph{diagram} is a finite rooted tree equipped with a partition
            of its nodes, such that the root node occupies a part of size $1$.
            Note that this definition generalizes the special case reported in
            the paper body; in particular, we no longer require the paper
            body's ``path condition'' to hold.  Thus, there are four diagrams
            with two edges:
            ...
            \begin{itemize}
                ...
                \item \textbf{correlation condition}: if two nodes are in the same
                    ...
                    datapoint $n$.
            \end{itemize}
            We consider the root note to be embedded to time $T$.
            ...
            its single-epoch embeddings.  In fact, this is true for all of the
            order-$2$ diagrams \emph{except} for $\sdia{c(01-2)(01-12)}$.
            Therefore, to second order, the answer to Question \ref{qst:multi}
            will be (some multiple of) $\sdia{c(01-2)(01-12)}$.  We now turn
            to the evaluation of this diagram.
    ...
    \subsection{How to identify the relevant space-time}            \label{appendix:draw-spacetime}
            Diagrams tell us about the loss landscape but not about parameters
            such as $T$.  We summarize SGD parameters as sets of pairs $(n,t)$,
            one for each participation of the $n$th datapoint in the $t$th
            ...
            We see that each column contains $B$ (batch size)
            many shaded cells and that each row contains $E$ (epoch number)
            many shaded cells.
            ...
            \begin{figure}[h!] 
                ...
                \caption{
                    ...
                    \textbf{The space-time grids of single- and multi-epoch
                    SGD.}
                }
                \label{fig:spacetimes}
            \end{figure}
            
            A space-time grid also depicts the permutation --- or lack thereof
            --- of training sets between epochs.  Since each grid commits to a
            concrete sequence of training set permutations, we may analyze
            SGD with randomized permutations by taking expectations over
            multiple space-time grids.  However, all of the corollaries in
            this text are invariant to inter-epoch training set permutations,
            so we will not focus on this point.\footnote{A routine check shows
    ...
    \subsection{How to identify the relevant diagram embeddings}    \label{appendix:draw-embeddings}
            ...
            \begin{figure}[h!] 
                \centering
                \dmoo{3.55cm}{spacetime-e}
                \caption{
                    Moo
                }
                \label{fig:embedding-rules}
            \end{figure}

            Specifically, let's compute how the \emph{skewness} $S$ affects
            SGD's test loss. The recipe is to identify the fewest-edged
            diagrams containing $S = \sdia{MOOc(012)(0-1-2)}$.  In this case,
            there is one fewest-edged diagram --- $\sdia{c(012-3)(03-13-23)}$;
            it results from joining $S$ with $J=\sdia{MOO(0)(0-0-0)}$.  To
            evaluate a diagram, we multiply its components (here, $S, J$) with
            exponentiated $\eta H$'s, one for each edge (here, there are three
            edges).  The result is easiest to write in terms of an eigenbasis
            of $\eta H$:
...
\section{Mathematics of the theory}\label{appendix:math}
    \subsection{Assumptions}                                        \label{appendix:assumptions}
