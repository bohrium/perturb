...
\section{Introduction}
    ...
    \subsection{Example of diagram-based reasoning}
        Our theory expresses SGD dynamics in terms of combinatorial objects we
        call \emph{diagrams}.  Deferring formal details to the following
        sections, we present a sample computation to illustrate how, by
        encoding tensors as diagrams, our theory yields non-trivial results via
        short arguments. 
        ...
        First, we establish a dictionary between components of diagrams and 
        statistics of the loss landscape:
        \begin{table}[h]\centering
            \begin{tabular}{ccc}
                $G \teq \ex{\nb\lx}       \teq \sdia{(0-1)(01)}           $ &                                                         \\
                $H \teq \ex{\nb\nb\lx}    \teq \sdia{(0-1-2)(02-12)}      $ & $C \teq \ex{(\nb\lx - G)^2} \teq \sdia{c(01)(01)}     $ \\
                $J \teq \ex{\nb\nb\nb\lx} \teq \sdia{(0-1-2-3)(03-13-23)} $ & $S \teq \ex{(\nb\lx - G)^3} \teq \sdia{c(012)(01-12)} $ 
            \end{tabular}
            \caption{
                {\bf Notation}.
                Throughout, $G, H, J$ denote the first, second, and third
                derivatives of the loss function.  We write $C, S$ for the
                second and third cumulants of the gradient distribution.  We
                ...
                %
                In the diagram notation, each $l_x$ corresponds to a node, 
                each $\nabla$ corresponds to a thin edge from that node, and
                the fuzzy ties connect the nodes whose $l_x$'s occur within the
                same expectation.
            }
        \end{table}

        \emph{How does non-Gaussian gradient noise affect test loss?}
        For simplicity, consider SGD initialized at a minimum $\theta_0$ of
        test loss and run for $1$ epoch ($T$ steps) with batch size $1$.  When
        the gradients have a Gaussian distribution, $S=0$.  Our theory shows
        how to compute the leading-order effect of $S$ on test loss.  First, we
        identify the smallest diagram containing $S$: in this case,
        $\sdia{c(012-3)(03-13-23)}$
        %--- then evaluate the diagram as
        %$$
        %    S
        %    \wrap{
        %        \int_{0 \leq t < T} 
        %            \exp\wrap{ t
        %                ((I - \eta H)^{\otimes 3} - I^{\otimes 3})
        %            }
        %    }
        %    J
        %$$
        Then, we evaluate the diagram.  If we work in a basis that diagonalizes
        $\eta H$, the diagram evaluates to:
        $$
            \sum_{\mu\nu\lambda}
                S_{\mu\nu\lambda}
                \frac{
        ...
        \emph{How does heterogeneous gradient noise affect learning?}
