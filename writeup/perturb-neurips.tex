...
\begin{abstract}
    ...
    %
    We present a novel analysis of Stochastic Gradient Descent (SGD) at small
    learning rates.
    %
    %-----------------  applications  -----------------------------------------
    %
    Our general theory yields several concrete corollaries: we prove that
    gradient noise systematically pushes SGD toward flatter minima.  We
    characterize when and why flat minima overfit less than sharp minima.  We
    generalize the Akaike Info.\ Criterion (AIC) to a smooth estimator of
    overfitting, hence enabling gradient-based model selection.  We
    show how non-stochastic GD with a modified loss function may emulate SGD.
    Without making a continuous-time approximation (whose errors we quantify),
    %we compute the effects of non-Gaussian gradient noise, and
    we refine and explain a previous continuous-time result that SGD may
...
\section{Introduction}
    ...
    GD \cite{bo91}.  In this paper, we refine that intuition to account for
    gradient noise.  We will find that gradient noise \emph{biases} learning
    toward certain areas of weight space. \
    %Our main contribution is to present a framework for analyzing this bias.
    %\emph{How does gradient noise bias learning?}.  
    ...
    %-----------------  experiments  ------------------------------------------
    %
    We prove our predictions for small learning rates, and our experiments (on
    loss landscapes such as CIFAR-10 convnets; details in Appendix
    \ref{sect:landscape}) show that even a single evaluation of our force laws
    ...
    Our novel formalism offers qualitative insight, as well: we interpret SGD
    as a superposition of several concurrent interactions between weights an
    data, each represented by a diagram that echoes the visual schemata of
    \citet{fe49, pe71}.  FILL IN
    %
    %-----------------  prospective  ------------------------------------------
    %
    In short, we present a general tool for studying stochastic optimization,
    especially on short timescales.  Our conclusion discusses Hessian methods
    and natural GD as topics for future.
    ...
    \subsection{Overview of the perturbative approach}
        ...
        diagram $\sdia{(0-1)(01)}$\footnote{%\label{note1}
    ...
    \subsection{A first look at curvature and noise}
        ...
        streamlines calculation, yielding arguments $4$ times shorter and
        arguably more illuminating than direct calculation.  The more
        complicated the computation, the greater the savings from diagrams.
...
\section{Background and Notation} \label{sect:background}
    ...
    \subsection{Tensor conventions}
        ...
        gradients.  By convention, we implicitly sum repeated Greek indices:
        if $A_\mu, B^\mu$ are the coefficients of a covector $A$ and a vector
        $B$\footnote{
    ...
    \subsection{Diagrams and embeddings}
        ...
        \begin{defn}[Diagrams] \label{dfn:diagrams}
            A diagram is a finite rooted tree equipped with a partition of its
            nodes.  We draw the tree using \emph{thin edges}.  By convention,
            ...
            within each part via \emph{fuzzy ties}.  For example,
...
\section{Diagram Calculus for SGD} \label{sect:calculus}
    ...
    \subsection{Recipe for SGD's test loss and generalization}
        ...
        \begin{table}[H]
            ...
            \caption{
                Degree-$3$ diagrams for $B=M=1$ SGD's test loss.  The $6$
                diagrams have $(4+2)+(2+2+3)+(1)$ total orderings relevant to
...
\section{Assumptions and Proofs}
    \subsection{Setup and assumptions of our theory}                %\label{subsect:tutorial}
    \subsection{Proof of the Key Lemma}                             %\label{subsect:tutorial}
    \subsection{Proof of Theorem 1}                                 %\label{subsect:tutorial}
    \subsection{On Mobius inversion for resummed diagrams}          %\label{subsect:tutorial}
    \subsection{Proof of Theorem 2}                                 %\label{subsect:tutorial}
    \subsection{Proofs of corollaries}                              %\label{subsect:tutorial}
    \subsection{Proofs of miscellaneous claims}                     %\label{subsect:tutorial}
