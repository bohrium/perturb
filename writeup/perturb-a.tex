...
\section{Background and Notation}
    ...
    \subsection{A Smooth Stage: Tensor Conventions}
        ...
        $C^\omega$\footnote{
            This assumption weakens to $C^k$ for our order-$k$ results.
        } expectations, and that expectations and derivatives commute
        throughout.  We use $G, H, J$ for the first, second, and third
        derivatives of $l$.
...
\section{Diagram Calculus for SGD}
    ...
    \subsection{Recipe for SGD's Test Loss and Generalization}
        ...
        \begin{thm}[Generalization Gap as a Path Integral] \label{thm:gen}
            ...
            formula \ref{eq:sgdcoef} with $\dvalue(D)$ replaced by
            $
    ...
    \subsection{Descent as Scattering}
        ...
        $f:D\to S$ of Theorem \ref{thm:test} as an embedding of the graph $D$
    ...
    \subsection{Effective Theories} \label{subsect:effective}
        ...
        that are \emph{irreducible} in the sense that they contain no
        $\sim$-singleton degree-two nodes (such as the middle node of
        $\sdia{(0-1-2)(01-12)}$).  The contribution of a diagram, as usual,
        will be a sum over all its embeddings into spacetime.  This time,
        though, the integrand will be exponentially weighted by $\exp(-\eta
        \Delta T H)$ for each edge of duration $\Delta T$.  One thus gets an
        integral of exponentials.

        \begin{thm}[Renormalization gives $T$-Uniform Convergence] \label{thm:renorm}
            The test loss of SGD has a MacLaurin series as in Theorem
            \ref{thm:test}, except with only irreducible diagrams included
            and for each embedding $f:D \to \Free(S)$ 
            ...
            \end{equation}
            and with the embedding-agnostic value $\dvalue(D)$ replaced by an
            embedding-sensitive value $\dvalue_f(D)$.  Here, $\dvalue_f(D)$
            has the same expression as $\dvalue(D)$ except each contraction  
            by $-\eta$ between nodes embedded to times $t, \tilde t$ is
            replaced by a factor $(I-\eta H)^{\wabs{t-\tilde t}} \eta$.
            The modification to compute the generalization gap is the same as
            in Theorem \ref{thm:gen}.
            When $H$ is positive and the loss landscape data is $C^\omega$, the
            above converges \emph{uniformly} over all $T$. 
            In practice, we prefer to replace sums by integrals and $(I-\eta
            H)^t$ by $\exp(- \eta H t)$, thus incurring a term-by-term
            multiplicative error of $1 + o(\eta)$.
        \end{thm}

        Because the renormalized theory has guarantees even for large $T$, we
        use it to probe some long-time behavior:
        ...
        This added generality allows us to observe a qualitatively new
...
\section{Experiments and Applications}
    ...
    \subsection{A Nonconservative Entropic Force} \label{subsect:entropic}
        ...
        Moreover, by modeling arbitrary covariances, we refine \citet{we19b}'s  
        result that SGD seeks minima flat with respect to
        $\eta^{-1}$ to the statement that SGD seeks minima that are flat with
        respect to $C$.  Unlike \citet{we19b}, our analysis does not require 
        hypotheses of thermal equilibrium, of fast/slow mode separation, or 
        of constant covariance; only our analysis produces 
        the phenomenon of a non-conservative velocity field.
        ...
        \begin{figure}[H]
            ...
            \caption{
                {\bf Left}: 
                {\bf Right}: Observe that the net displacement well exceeds the
                $z$-periodicity of $2\pi$. 
            }
        \end{figure}
    ...
    \subsection{Sharp and Flat Minima Both Overfit Less} \label{subsect:overfit}
        ...
        FIGURE FILL IN intuitively explains these singularities.  Fortunately,
        as calculated in Proposition \ref{prop:overfit}, the implicit
        ...
        Because matrix exponentiation is expensive, STIC regularization is
        useful only for fitting moderate or low-dimensional models to very
        scanty or noisy data.
...
\section{Conclusion}
    ...
    We showed that Feynman diagrams make novel and precise predictions about
    SGD.  

    %\par\noindent\framebox{\parbox{\dimexpr\linewidth-2\fboxsep-2\fboxrule}{
        \textsc{Story of $\sdia{c(01-2)(02-12)}$:~}
        Flat and sharp minima both overfit less than minima of medium
        curvature, i.e. curvature comparable to $\eta^{-1}/T$.
    %}}
    ...
    %\par\noindent\framebox{\parbox{\dimexpr\linewidth-2\fboxsep-2\fboxrule}{
        ...
        evolves and thus the effective landscape evolves.  This generically
        leads to nonconservative SGD dynamics.  This SGD dynamics does not need
        to converge to a periodic limit cycle. 
    %}}
    ...
    For example, it offers a conceptual understanding of the empirically
    verified entropic forces at play in minimum valleys, and in emphasizing the
    underlying metric, it reconciles competing views of whether sharp or flat
    minima generalize.  This bridge to physics invites further exploration.
    With the diagram method established, it is especially interesting to
    develop toward Lagrangian methods and curved backgrounds: 
    \begin{quest}
...
\section*{D. Diagram Rules vs Direct Perturbation} \label{sect:compare}
    ...
    Despite these efforts, the diagram method yields arguments about five times
...
\section*{F. Loss Landscapes Used for Experiments}
    ...
    fact, test and train sets overlap but in reference, not in sense: the
    ...
    words, say, ``Thank you!''.  Though the special structure of finite-support
    distributions is convenient for experiments, our theory makes no use of
    this structure.  In any case, all of our experiments focus on the
