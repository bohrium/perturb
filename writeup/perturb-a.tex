...
\section*{A. Derivation of Diagram Rules}
    ...
    \subsection*{Error Rates for the Renormalized Theory }
        ...
        proof and results of \ref{thm:sgdcoef}.  In particular, the
        coefficients $c_T(d)$ on $\eta^d$ must decay exponentially with $d$.

        To prove the rest of Theorem \ref{thm:renorm}, we assume that the
        $l_x$s are strictly convex with a positive lower bound on curvature
        uniform in $x$ but potentially dependent on $\theta$.  By this
        assumption, we may choose $T$ so large that each coefficient $.$ 
