...
\section{Background and Notation}
    ...
    \subsection{A Smooth Stage: Tensor Conventions}
        ...
        smooth expectations, and that expectations and derivatives commute
    ...
    \subsection{Combinatorial Costumes: Structure Sets}
        ...
        $D$, where $P$ counts $D$'s equivalence classes.  Let
        $|\edges(D)|$ and $\ords(D)$ count edges and orderings of
        $D$.
...
\section{Diagram Calculus for SGD}
    ...
    \subsection{The Role of Diagrams}
        ...
        the fuzzy ties.  The value $\dvalue(D)$ of a diagram $D$ is the
        expected value of the corresponding tensor expression.
    ...
    \subsection{Descent as Scattering}
        ...
        $f:D\to S$ (Theorem \ref{thm:test}) as an embedding of the graph $D$
    ...
    \subsection{Effective Theories} \label{subsect:effective}
        ...
        \begin{thm}[Renormalization] \label{thm:renorm}
            The test loss of SGD has a MacLaurin series as in
            \label{thm:test}, except with only irreducible diagrams included
        ...
        TODO: discuss benefits of Renormalization Theorem
        ...
        \begin{prop}[A Nonconservative Entropic Force]\label{prop:entropic}
            ...
            order $\eta^2$ with a long-time-averaged\footnote{
                That is, $T$ so large that $\exp(-\eta H T)$ is vanishing.
...
\section{Conclusion}
    ...
    Most of the calculations we presented here using our technique can be means
    of this technique be done fully in one's head; in fact, we have answered
    several natural questions that are hard to do in direct perturbation.  We
    see this as evidence that our method is helpful.
...
\section*{F. Loss Landscapes Used for Experiments}
    In addition to the pedagogically artificial loss landscapes (Gaussian Fit,
    ...
    initialization of the linear maps in each layer pulled back to standard
    normal initializations of the parameters. 
\end{document}
