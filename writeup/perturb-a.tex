...
\subsection{Descent as Scattering}
    ...
    \begin{figure}[h!]
        \centering
        \plotmoo{diagrams/spacetime}{8.0cm}{3.0cm} 
        %\plotplace{one diagram, many embeddings}{4.0cm}{4.0cm} \\
        %\plotplace{interepoch shuffling}{4.0cm}{4.0cm}
        %\plotplace{multiepoch vs gd}{4.0cm}{4.0cm}
        \caption{
            {\bf a} Some diagrams embedded in spacetime.  Left four 
                    give order $\eta^1$, $\eta^1$, $\eta^3$, and $\eta^5$
                    contributions to GD's test loss.  Right four each contribute $\eta^2
                    \expct{\nabla^2} \expct{\nabla}^2$; their
                    equivalence demonstrates crossing symmetry.  Only diagrams
                    whose nodes fall within shaded diagonals contribute to
                    SGD's test loss (with an extra factor $N$ per edge). 
            {\bf b}   
            {\bf c} 
            {\bf d} 
        }
