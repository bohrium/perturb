...
\newpage
\section{How to calculate test losses}                   \label{appendix:tutorial}
    ...
    objects --- \emph{space-time grids} --- arise as we relax the paper body's
    ...
    \subsection{How to identify the relevant diagram embeddings}    \label{appendix:draw-embeddings}
        ...
        {\color{red} TODO: in the diagram section, talk about
        irreducibility}
        ...
        non-zero number of embeddings into the relevant space-time.  For each
    ...
    \subsection{How to evaluate each embedding}                     \label{appendix:evaluate-embeddings}
        ...
        \subsubsection{Un-resummed values: $\uvalue(D)$}
            ...
            \begin{exm}
                For example, if we temporarily denote moment values by solid
            ...
            %\begin{exm}
            %    This example is more complicated than any we will actually use.
            %    Moreover, the depicted graphs are not actally diagrams because
            %    their edges don't form trees.  However, we include this example
            %    because it depicts in one place all the features of the node
            %    and edge rules.

            %    Remember that
            %    $
            %        \mdia{MOOc(012)(0-1-2)} = S_{\mu\nu\rho}
            %    $, 
            %    $
            %        \mdia{MOOc(0)(0-0)} = H_{\lambda\pi}
            %    $, and
            %    $
            %        \mdia{MOOc(0)(0)} = G_{\sigma}
            %    $, so that
            %    $$
            %        \mdia{MOOc(012)(0-1-2)} 
            %        \mdia{MOOc(0)(0-0)}
            %        \mdia{MOOc(0)(0)}
            %        = S_{\mu\nu\rho} H_{\lambda\pi} G_{\sigma}
            %    $$
            %    Then
            %    $$
            %        \mdia{MOOc(012-3-4)(01-23-34)}
            %        = S_{\mu\nu\rho} H_{\lambda\pi} G_{\sigma}
            %          \eta^{\mu\nu}
            %          \eta^{\rho\lambda}
            %          \eta^{\pi\sigma}
            ...
            %    and, in contrast,
            %    $$
            %        \mdia{MOOc(012-3-4)(03-13-24)}
            %        = S_{\mu\nu\rho} H_{\lambda\pi} G_{\sigma}
            %          \eta^{\mu\lambda}
            %          \eta^{\nu\pi}
            %          \eta^{\rho\sigma}
            %    $$
            %    The two graphs have different values because their
            %    edges connect the nodes in different ways.  \mend
            %\end{exm}
