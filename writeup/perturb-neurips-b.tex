...
\section{Background and Notation} \label{sect:background}
    ...
    \subsection{Diagrams and embeddings}
        ...
        \begin{defn}[A Diagram's Un-resummed Value]
            ...
            We write $\dvalue(D)$ for $D$'s un-resummed value, or $D$ when
            clear.
        \end{defn}
...
\section{Diagram Calculus for SGD} \label{sect:calculus}
    ...
    \subsection{Single-Epoch, Singleton-Batch SGD}
        ...
        \end{prop}
        So a diagram with $d$ thin edges and $c$ fuzzy ties (hence $d+1-c$
        parts), contributes $\Theta\wrap{(\eta T)^d T^{-c}}$ to SGD's test
        loss. 
        %
...
\section{Insights from the Formalism}
    ...
    \subsection{Effects of epochs and of batch size}
    ...
    \subsection{SDE is blind to non-Gaussian noise}
        ...
        \begin{cor}[SGD Differs from ODE, SDE] \label{cor:vsode}
            ...
            $
                \frac{T}{2} ~ C_{\mu\nu} H^{\mu\nu} + o(\eta^2)
            $.
            Its leading order deviation from SDE is due to non-Gaussian noise:
            $
