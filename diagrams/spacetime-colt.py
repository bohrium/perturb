...
def draw_line_aa(img, row_s, col_s, row_e, col_e, color=black):
    ...
    #print(img.shape, val.shape, expanded_color.shape)
    img[rr, cc, :] = expanded_color #np.minimum(img[rr, cc, :], 
            #np.expand_dims(val, 2) * expanded_color)
            #np.expand_dims(val, 1) * expanded_color)
...
def draw_grid(img): 
    for n in range(N):  
        # horizontals:
        print(get_r(n+0.05), get_c(0), get_r(n+0.05), get_c(T), height, width)
...
FILE_NM = 'spacetime-b1-e2-nosh-populated.png' 
#title = "One-Epoch SGD"
